...
export const UpdatePatientComponent = ({
  setToUpdatePatient,
  toUpdatePatient,
  setReload,
  reload,
}: {
  setToUpdatePatient: Dispatch<SetStateAction<IPatient | null>>;
  toUpdatePatient: IPatient;
  ...
  const handleUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const age = Number((form.age as HTMLInputElement).value);
    ...
    fetch(`${envConfig.baseApi}/patient/update/${toUpdatePatient?._id}`, {
      method: "PUT",
      headers: {
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-lg z-50 p-4">
      <div className="w-full max-w-2xl bg-white rounded-xl shadow-2xl overflow-hidden animate-fadeIn">
        ...
        <div className="bg-teal-600 px-6 py-4 flex justify-between items-center">
          <h2 className="text-xl font-semibold text-white">Update Patient</h2>
          <button
            className="text-white hover:text-teal-200 transition"
            onClick={() => setToUpdatePatient(null)}
          >
            <X size={24} />
          </button>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                ...
                <input
                  type="text"
                  name="fullName"
                  defaultValue={toUpdatePatient?.fullName}
                  required
              ...
              <div>
                ...
                <select
                  name="gender"
                  defaultValue={toUpdatePatient?.gender}
                  required
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition"
                >
              ...
              <div>
                ...
                <input
                  type="number"
                  name="age"
                  defaultValue={toUpdatePatient?.age}
                  required
                  min="0"
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition"
                />
              ...
              <div>
                ...
                <input
                  type="tel"
                  name="phoneNumber"
                  defaultValue={toUpdatePatient?.phoneNumber}
                  required
                  pattern="\d{11}"
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition"
                />
              ...
              <div>
                ...
                <input
                  type="email"
                  name="email"
                  defaultValue={toUpdatePatient?.email}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition"
                />
              </div>
              ...
              <div className="col-span-2">
                ...
                <textarea
                  name="address"
                  defaultValue={toUpdatePatient?.address}
                  rows={3}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition"
                />
              </div>
